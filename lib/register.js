...
async function ensure_login() {
  ...
  console.log('登陆短信平台...')
  const data = await request_sms('loginIn', { name: config.sms_platform.key, password: config.sms_platform.password })
...
async function get_phone(ctx) {
  ...
  console.log(`获取手机号...`)
  const data = await request_sms('getPhone', { sid: config.sms_platform.sid, token })
...
async function get_code(ctx) {
  ...
  while (true) {
    const data = await request_sms('getMessage', { sid: config.sms_platform.sid, phone: ctx.phone, token })
    try_count++
