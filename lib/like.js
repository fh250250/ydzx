...
function delete_comment(comment) {
  ...
  return collection.deleteMany({ docid: comment.docid })
}
...
function delete_proxy(proxy) {
  ...
  return coll.deleteOne({ host: proxy.host, port: proxy.port })  
}
...
async function calc_all_comments_count() {
  ...
  return _.zipWith(
    ...
    (comment, limit) => Object.assign(comment, { limit, like: 0, exist: true })
  )
}
...
function like_comment(user, proxy, comment) {
  return axios.get('http://a1.go2yd.com/Website/interact/like-comment', {
    headers: {
      'user-agent': faker.internet.userAgent,
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    if (comment.exist && comment.like < comment.limit) {
      try {
        ...
        console.log(`❌ ${e.message} ${proxy.host}:${proxy.port}`)

        if (/找不到评论/.test(e)) {
          comment.exist = false
          await delete_comment(comment)
        }
        else if (/重复提交/.test(e)) { comment.exist = false }
        else {
